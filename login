...
*  TRY .
*
**► select query to fetch employee details from table ◄
*      SELECT SINGLE
*        employeesalesorg
*        employeesalesoffice
*        employeesalesgroup
*        employeedistributionchannel
*        employeedivision
*        FROM zempsalesmodel
*        INTO
*        (ev_emp_sales_org,
...
*         ev_emp_distr_chnl,
*         ev_emp_division    )
*        WHERE employeeid EQ ev_person_id.
*      IF sy-subrc NE 0.
*        ev_query_message = hc_class->fail_flag.
*      ELSE .
*        ev_query_message = hc_class->success_flag.
*      ENDIF.
*
*    CATCH cx_sql_exception   .

**fetch all roles related with that user
*  SELECT agr_name
*    FROM agr_users
*    INTO TABLE lt_agr_users
*    WHERE uname EQ iv_username.
*
*  IF lt_agr_users IS NOT INITIAL.
**fetch organizational data related to that role
*    SELECT
*      agr_name
*      field
*      low
*    FROM  agr_1251
*    INTO TABLE lt_sales_org
*    FOR ALL ENTRIES IN lt_agr_users
*    WHERE  agr_name = lt_agr_users-agr_name
*      AND
*     (
*      field EQ 'DIS_CHANNE' OR
*      field EQ 'SALES_GROU' OR
*      field EQ 'SALES_OFFI' OR
*      field EQ 'SALES_ORG' )
*        .
*
*  ENDIF.
*
*  LOOP AT lt_sales_org INTO lt_sales_org .
**condition to check if sales org have * value
*    IF ( lt_sales_org-field = 'SALES_ORG' AND lt_sales_org-low = '*' ).
**if * value is found set flag
*      flag_star = '1'.
*      EXIT.
*    ENDIF.
*endloop.
...
*LOOP AT lt_sales_org INTO lt_sales_org .
**condition to check if sales org have * value
*    IF ( lt_sales_org-field = 'SALES_OFFI' AND lt_sales_org-low = '*' ).
**if * value is found set flag
*      flag_star_office = '1'.
*      EXIT.
*    ENDIF.
...
*IF flag_star ne '1' .
*LOOP AT lt_sales_org INTO lt_sales_org .
** if * not found then create list of all sales orgs in itab
*    IF ( lt_sales_org-field EQ 'SALES_ORG' AND lt_sales_org-low NE '*' ) .
*      APPEND lt_sales_org-low TO lt_slorgs.
*    ENDIF.
*endloop.
*endif.
*
*IF flag_star_office ne '1' .
*LOOP AT lt_sales_org INTO lt_sales_org .
*    IF ( lt_sales_org-field = 'SALES_OFFI' and lt_sales_org-low ne '*' ).
*      APPEND lt_sales_org-low to lt_sloffices.
*    endif.
*  ENDLOOP.
*ENDIF.
*
*  IF lt_slorgs IS NOT INITIAL.
**sort and delete duplicates
*    sort lt_slorgs ASCENDING BY low.
*    DELETE ADJACENT DUPLICATES FROM lt_slorgs.
**    delete lt_slorgs WHERE low eq ' ' .
*  ENDIF.
*
** if sales org value is *
*  IF flag_star EQ '1' .
*
...
*  CHANNEL
*  DIVISION
*  FROM CRMV_BP_FRG0140
*  INTO TABLE ot_emp_login
* .
*
**if sales org value is not *
*  ELSEIF flag_star EQ '0' .
*
*    IF lt_slorgs[] IS NOT INITIAL .
*
**select assignments only for listed sales org
*SELECT
*  SALES_ORG
*  SALES_OFFICE
*  SALES_GROUP
...
*  FROM CRMV_BP_FRG0140
*  INTO TABLE ot_emp_login
*  FOR ALL ENTRIES IN lt_slorgs
*  WHERE sales_org EQ lt_slorgs-low .
*
*ENDIF.
*
*ENDIF.
*
*      LOOP AT ot_emp_login INTO wa_ot_tab .
*
**sales org description
*        IF wa_ot_tab-sl_org IS NOT INITIAL .
*          SELECT SINGLE stext
*            from HRP1000
*            INTO wa_ot_tab-sl_org_desc
*            WHERE
*            otjid eq wa_ot_tab-sl_org AND
*            langu eq 'E' AND
*            plvar eq '01' and
*            begda eq (
*                      SELECT max( begda )
*                          FROM HRP1000
*                          WHERE otjid eq wa_ot_tab-sl_org AND
*                          langu eq 'E' AND
*                          plvar eq '01' ).
*        ENDIF.
*
*  IF wa_ot_tab-sl_office IS NOT INITIAL .
**sales office description
*    SELECT SINGLE stext
*  from HRP1000
*  INTO wa_ot_tab-sl_office_desc
*  WHERE
*  otjid eq wa_ot_tab-sl_office AND
*  langu eq 'E' AND
*  plvar eq '01' and
*  begda eq (
*              SELECT max( begda )
*              FROM HRP1000
*              WHERE otjid eq wa_ot_tab-sl_office AND
*                    langu eq 'E' AND
*                    plvar eq '01' ).
*
*
...
*
*
*  IF wa_ot_tab-sl_group IS NOT INITIAL .
**sales group description
*  SELECT SINGLE stext
*  from HRP1000
*  INTO wa_ot_tab-sl_group_desc
*  WHERE
*  otjid eq wa_ot_tab-sl_group AND
*  langu eq 'E' AND
*  plvar eq '01' and
*  begda eq (
*              SELECT max( begda )
*              FROM HRP1000
*              WHERE otjid eq wa_ot_tab-sl_group AND
*                    langu eq 'E' AND
*                    plvar eq '01' ).
*
*
*        ENDIF.
*
*IF wa_ot_tab-disrt_chnl is not INITIAL  .
**distribution channel description
*SELECT SINGLE DESCRIPTION
*  FROM CRMC_DISTCHAN_T
*  INTO wa_ot_tab-disrt_chnl_desc
*  WHERE DISTCHAN eq wa_ot_tab-disrt_chnl and
*  langu EQ 'EN'.
*
*ENDIF.
*
*IF wa_ot_tab-emp_division is not initial .
** sales division description
*SELECT SINGLE DESCRIPTION
*  FROM CRMC_DIVISION_T
*  INTO wa_ot_tab-emp_division_desc
*  WHERE DIVISION EQ wa_ot_tab-emp_division AND
*        langu EQ 'EN'.
*
*ENDIF.
*
**update work area to itab
*modify OT_EMP_LOGIN FROM wa_ot_tab
*  TRANSPORTING
*  sl_org_desc
*  DISRT_CHNL_DESC
*  sl_office_desc
...
**sort out tab
*sort OT_EMP_LOGIN DESCENDING BY sl_org sl_office sl_group disrt_chnl emp_division.
